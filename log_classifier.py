...
from llm_engine import classify_with_llm

from config import REGEX_CONFIG_PATH

regex_classifier = RegexClassifier(REGEX_CONFIG_PATH)
bert_log_classifier = LogClassifier()
...
def classify_log(source, log_msg):
    if source == "LegacyCRM":
        ...
        label = regex_classifier.classify(log_msg)
        if not label:
            label = bert_log_classifier.predict(log_msg)
    return label
